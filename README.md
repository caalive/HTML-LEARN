...
- 浮动流排版
 ...
 - 浮动元素的排序规则
  - 相同方向上的浮动元素，先浮动的元素会先显示在前面，后浮动的元素会显示在后面。
  - 不同方向上的浮动元素左浮动会找左浮动，有浮动会找右浮动。
  - 浮动元素浮动之后的位置，由浮动元素之前在标准流中的位置决定。
 - 浮动元素贴靠现象
 	- 当父元素的宽度不足以放下浮动的子元素时，此时子元素会从最后一个浮动的元素开始往前贴靠，如果贴靠的前一个元素宽度还不能容纳这个浮动的元素，就会一直往前贴靠直到可以为止。
 - 浮动元素的字围现象
 	- 应用场景做图文混排
 	- 当一个元素设置为浮动元素后，另外一个元素没有设置浮动，则浮动元素会盖住没有设置浮动元素的那个元素，此时在没有设置浮动的那个元素中添加文字，这些文字不会被设置为浮动元素的那个元素所覆盖。
 - 在一般企业开发中，垂直方向上的布局一般使用标准流来布局，水平方向上的布局一般使用浮动流来布局。
 - 浮动元素的高度问题
 	- 在标准流中内容的高度可以撑起父元素的高度。
 	- 在浮动流中，浮动的元素是不可以撑起父元素的高度的。
 - 清除浮动元素高度
 	- 给前一个父元素设置高度，一般在企业开发中能不写高度就不写高度。
 	- 清除浮动的第二种方式，给后面的元素添加一个clear属性，clear属性取值有四种
 	- none 默认取值，按照浮动元素的排序规则来排序，左浮动找左浮动，右浮动找右浮动
 	- left 不找浮动元素左边的浮动元素
 	- right 不找浮动元素右边的浮动元素
 	- both 不找左右浮动的元素
 	- 注意点：
 		- 设置了clear both属性后 margin top 属性会失效。
 	- 清除浮动方式三
 		- 外墙法
 			- 在两个盒子中间添加一个块级元素，给这个元素添加一个 claer：both 属性
 			- 外墙法可以让第二个盒子使用 margin-top 属性，但是不能让第一个盒子使用 margin-bottom 属性
 		- 内墙法
 			- 在第一个盒子中所有子元素最后添加一个额外的块级元素，并给这个额外添加的块级元素设置clear:both属性。
 			- 注意点
